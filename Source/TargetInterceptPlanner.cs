...
namespace TargetInterceptPlanner
{
    internal static class Util
    {
        ...
        // Math.Acosh does not exist in .NET 4
    ...
    public class TargetInterceptPlanner : MonoBehaviour
    {
        ...
        double flightTime_Adj = double.NaN; // Desired total flight time from launch to intercept, in whatever unit the user has selected
        ...
        bool useAltBehavior = true; // Find global minimum for low latitudes instead of local minimum
        ...
        double CalculateLaunchTime(Vector3d launchPos, double flightTime, double startTime, bool useAltBehavior, bool? overrideNaN)
        {
            ...
            double AzimuthError(double candidateTime) // this doesnt actually spit out the azimuth error since we're using cos, but it does spit out the right error
            {
                ...
                return 1d - Math.Cos(2d * (az - targetLaunchAzimuth) * degToRad); // 0 and 180 are considered the same, 180 and 360 are considered the same
            }
            ...
            if (useAltBehavior)
            {
                ...
                while (candidateTime <= startTime + maxTimeLimit)
                {
                    ...
                    else // global min not found yet
                    {
                        ...
                        {
                            bool NaNBehavior = overrideNaN ?? altBehaviorNaN;
        ...
        private (double phasingTime, double phasingAngle) CalculateTimeBeforeManeuver(double flightTime, double startUT, double transferEcc, double inclination, double LAN, double AoP)
        {
            ...
            double DistanceError(double phasingAngle)
            {
                ...
                LogDebug($"CalculateTimeBeforeManeuver stopwatchDistanceError1: {stopwatchDistanceError.Elapsed.TotalSeconds}s");

                transferOrbit.argumentOfPeriapsis = Util.ClampAngle(AoP + phasingAngle, false);
                transferOrbit.epoch = epoch;

                LogDebug($"CalculateTimeBeforeManeuver stopwatchDistanceError2: {stopwatchDistanceError.Elapsed.TotalSeconds}s");

                LogDebug($"inclination: {inclination}, transferEcc: {transferEcc}, transferSMA: {transferSMA}, LAN: {LAN}, argumentOfPeriapsis: {Util.ClampAngle(AoP + phasingAngle, false)}, epoch: {epoch}, meanAnomaly: {meanAnomaly}, flightTime: {flightTime}");
                ...
                {
                    LogDebug($"A value is NaN! meanAnomaly: {meanAnomaly}, transferEcc: {transferEcc}");
                }

                LogDebug($"CalculateTimeBeforeManeuver stopwatchDistanceError3: {stopwatchDistanceError.Elapsed.TotalSeconds}s");
                ...
                LogDebug($"CalculateTimeBeforeManeuver stopwatchDistanceError4: {stopwatchDistanceError.Elapsed.TotalSeconds}s");
                ...
                LogDebug($"CalculateTimeBeforeManeuver stopwatchDistanceError5: {stopwatchDistanceError.Elapsed.TotalSeconds}s");

                double distance = Vector3d.Distance(targetPos, futurePos);

                stopwatchDistanceError.Stop();
                LogDebug($"CalculateTimeBeforeManeuver stopwatchDistanceErrortotal: {stopwatchDistanceError.Elapsed.TotalSeconds}s\n");
            ...
            // therefore, we need to check for the best angle in both ranges, and then return the one with the lowest distance error
        ...
        private (double dV, double eccentricity, int errorStateDV) CalculateDV(double postManeuverTime, double startUT, double inclination, double LAN, double AoP)
        {
            ...
            double DistanceError(double eccentricity)
            {
                ...
                LogDebug($"CalculateDV stopwatchDistanceError1: {stopwatchDistanceError.Elapsed.TotalSeconds}s");

                transferOrbit.eccentricity = eccentricity;
                transferOrbit.semiMajorAxis = SMA;

                LogDebug($"CalculateDV stopwatchDistanceError2: {stopwatchDistanceError.Elapsed.TotalSeconds}s");

                transferOrbit.Init();
                transferOrbit.UpdateFromUT(startUT);

                LogDebug($"CalculateDV stopwatchDistanceError3: {stopwatchDistanceError.Elapsed.TotalSeconds}s");
                ...
                LogDebug($"CalculateDV stopwatchDistanceError4: {stopwatchDistanceError.Elapsed.TotalSeconds}s");
                ...
                LogDebug($"CalculateDV stopwatchDistanceError5: {stopwatchDistanceError.Elapsed.TotalSeconds}s");

                Vector3d futurePos = transferOrbit.getPositionFromEccAnomaly(eccAnomaly);

                LogDebug($"CalculateDV stopwatchDistanceError6: {stopwatchDistanceError.Elapsed.TotalSeconds}s");

                double distance = Vector3d.Distance(targetPos, futurePos);

                stopwatchDistanceError.Stop();
                LogDebug($"CalculateDV stopwatchDistanceErrortotal: {stopwatchDistanceError.Elapsed.TotalSeconds}s\n");
            ...
            (double flightTime, double startTime, double inclination, double LAN, double AoP) // the inclination needs to be the one from launchOrbit
        {
            ...
            double eccentricity = 0.5d; // this gets increased or reduced later if needed, we just cant pick 0 or 1 to start
                ...
                for (; i < maxIterations; i++)
                {
                    ...
                    if (double.IsNaN(phasingTime) || double.IsNaN(phasingAngle))
                    {
                        if (eccentricity > 1) // if its NaN while eccentricity is above 1, that means the flight time is way too low
                        {
                            ...
                            LogDebug("NaN while eccentricity is above 1, setting errorStateDV to 2");
                            errorStateDV = 2;
                            break;
                        }
                    ...
                    {
                        LogDebug($"Eccentricity ({eccentricity}) is close to lastEcc ({lastEcc})");
                        break;
                    }

                    if ((lastEcc > 1 && eccentricity < 1))
                    {
                        LogDebug($"Switching between hyperbolic and eccentric! lastEcc: {lastEcc}, eccentricity: {eccentricity}");
                        errorStateDV = 1;
                        break;
                    }
                    ...
                    {
                        LogDebug($"Eccentricity was below minimum! eccentricity: {eccentricity}, minEccentricity: {minEccentricity}, lastEcc: {lastEcc}"); // gets falsely triggered with small flight times when it escapes previous loop?
            ...
            {
                LogDebug($"dV is above the maximum of {maxPossibleDV} for this body, returning NaN. (eccentricity: {eccentricity}, maxEccentricity: {maxEccentricity})");
                errorStateDV = 2;
            }
            ...
            { // check against hohmann maneuver time at PeR to make sure we have a sensible manuever time
                LogDebug($"dV is below the minimum possible to reach the target at this parking altitude, returning NaN.");
                errorStateDV = 1;
            }
    ...
    (double flightTime, double startTime, double inclination, double LAN, double initialAoP)
        ...
        { // targets with an eccentricity of 1 have already been filtered out
            ...
            if (referenceTimeMode == 0)
            {
                double FlightTimeError(double time)
                {
                    ...
                    (double LAN, double AoP) = CalculateLAN(0, latitude, longitude, azimuth);
                    int errorStateDV = CalculatePhasingAndDeltaV(time, 0, inclination, LAN, AoP).errorStateDV;

                    if (errorStateDV == 1) // flight time is too high
                    {
                        time = -absoluteMaxTime; // this makes the error very large so that the left side gets picked instead
                    }
            ...
            else
            {
                ...
                double FlightTimeError(double time)
                {
                    ...
                    if (double.IsNaN(candidateLaunchETA) || errorStateDV == 1) // unable to find a window or flight time is too high
                    {
                        time = -absoluteMaxTime; // this makes the error very large so that the left side gets picked instead
                    }
        ...
        private double GetCachedLaunchTime(Vector3d launchPos, double latitude, double longitude, double flightTime, double targetInclination, bool useAltBehavior, int windowNumber, bool? overrideNaN = null)
        {
            ...
            for (int w = windowCache.Count; w <= windowNumber; w++)
            {
                ...
                if (PrincipiaInstalled && Math.Abs(newLaunchTime - (lastLaunchTime - currentUT)) < 60d * dayScale)
            ...
            if (windowNumber.HasValue)
            {
                ...
                else
                {
                    ...
                    { // this is specifically a problem with targetLaunchAzimuth being 0, 180, or 360, and the azimuth at that time being 180 degrees apart
                      // so we have to wiggle back and forth until we find the right window (CalculateLaunchTime doesnt need to do this bc it uses Cos)
        ...
        private void MakeNumberEditField<T>(string controlId, ref T value, IConvertible step, IConvertible minValue, IConvertible maxValue, bool wrapAround = false) where T : struct, IConvertible
        {
            ...
            {
                double tick = Time.realtimeSinceStartup; // TODO, account for game lag which can cause the button to repeat itself (use currentUT?)
                if (tick > nextTick)
                {
                    int decimals = -1;
